ellipsoid(X) = get_parameters(datum_ellipse(X)) # for interfacing

# LLA -> ECEF
@inline function geotransform{oT}(::Type{oT}, cs_out::Union{LL_CS, LLA_CS}, datum_out, Xin, cs_in::ECEF_CS, datum_in, ::Type{CoordinateTransformsHandler})

    # get the ellipse
    elliptic_datum = combine_datums(datum_out, datum_in)

    # build the transform
    Xout = transform(LLAfromECEF(elliptic_datum), Xin)

    # build output
    assemble_position(oT, (Xout[2], Xout[1], Xout[3]), CRS(cs_out, elliptic_datum))
end


# ECEF -> LLA
@inline function geotransform{oT}(::Type{oT}, cs_out::ECEF_CS, datum_out, Xin, cs_in::Union{LL_CS, LLA_CS}, datum_in, ::Type{CoordinateTransformsHandler})

    # get the ellipse
    elliptic_datum = combine_datums(datum_out, datum_in)

    # build the transform
    Xout = transform(ECEFfromLLA(elliptic_datum), Xin)

    # build output
    assemble_position(oT, Xout, CRS(cs_out, elliptic_datum))
end
...
@inline function geotransform{oT}(::Type{oT}, cs_out::ECEF_CS, datum_out, Xin, cs_in::Union{LL_CS, LLA_CS}, datum_in, ::Type{CoordinateTransformsHandler})
    ...
    elliptic_datum = combine_datums(datum_in, get_datum(get_crs(refpos)))

    # build the transform
    Xout = transform(ENUfromECEF(origin, elliptic_datum), Xin)

    # build output
    assemble_position(oT, Xout, CRS(cs_out, datum_out))
end
...
immutable LLAfromECEF{Datum} <: AbstractTransformation{LLA, ECEF}
    ...
    function LLAfromECEF(a,f,e2,e2m,e2a,e4a,datum)
        println("Making an LLAfromECEFf")
        if !(isfinite(a) && a > 0)
